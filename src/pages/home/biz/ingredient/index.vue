	...
	<div class="layout-padding h-full overflow-hidden">
		<div class="layout-padding-auto layout-padding-view h-full overflow-hidden flex flex-col">
			...
			<el-row class="ml10" v-show="showSearch">
				<el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList" ref="queryRef">
					<el-form-item :label="$t('ingredient.name')" prop="name">
						<el-input class="!w-40" :placeholder="$t('ingredient.inputNameTip')" @keyup.enter="getDataList" clearable
							v-model="state.queryForm.name" />
					</el-form-item>
					<el-form-item :label="t('ingredient.unitId')" prop="unitId">
						<el-select class="!w-40" v-model="state.queryForm.unitId" :placeholder="$t('ingredient.unitId')" clearable>
			...
			<el-table :data="state.dataList" @selection-change="handleSelectionChange" style="width: 100%" row-key="id"
				...
				<el-table-column :label="$t('ingredient.name')" prop="name" show-overflow-tooltip></el-table-column>
				<el-table-column :label="$t('ingredient.unitName')" prop="unitName" />
...
import { delObj, fetchList } from '@/api/biz/ingredient';
