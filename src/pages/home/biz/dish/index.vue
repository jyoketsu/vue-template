...
import { delObj, fetchList } from '@/api/biz/dish';
	...
	<div class="layout-padding h-full overflow-hidden">
		<div class="layout-padding-auto layout-padding-view h-full overflow-hidden flex flex-col">
			...
			<el-row class="ml10" v-show="showSearch">
				<el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList" ref="queryRef">
					...
					<el-form-item :label="t('dish.categoryId')" prop="categoryId">
						<el-select class="!w-40" v-model="state.queryForm.categoryId" :placeholder="$t('dish.categoryId')"
							clearable>
			...
			<el-table :data="state.dataList" @selection-change="handleSelectionChange" style="width: 100%" row-key="id"
				...
				<el-table-column :label="$t('dish.cover')" prop="cover">
					<template #default="scope">
						<div class="size-full flex items-center">
							<el-image v-if="scope.row.cover" :src="scope.row.cover" class="w-10 h-10 rounded" fit="cover" />
							<p v-else class="text-gray-500">暂无封面</p>
						</div>
					</template>
				</el-table-column>
				<el-table-column :label="$t('dish.name')" prop="name" show-overflow-tooltip></el-table-column>
				<el-table-column :label="$t('dish.categoryName')" prop="categoryName" show-overflow-tooltip />
				<el-table-column :label="$t('dish.description')" prop="description" show-overflow-tooltip />
		...
		<table-form ref="formDialogRef" :category="category" @refresh="getDataList" />
	</div>
</template>
